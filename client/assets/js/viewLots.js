(function () {
  ...
  const fallbackFormatPhone = (value) => {
    ...
    const appendExtension = (formatted) => extension ? `${formatted} ${extension}` : formatted;
    if (core.startsWith('+1') || (digits.length === 11 && digits.startsWith('1'))) {
      const national = digits.slice(-10);
      if (national.length === 10) {
        return appendExtension(`+1 (${national.slice(0, 3)}) ${national.slice(3, 6)}-${national.slice(6)}`);
      }
    }
    if (!core.startsWith('+') && digits.length === 10) {
      return appendExtension(`(${digits.slice(0, 3)}) ${digits.slice(3, 6)}-${digits.slice(6)}`);
    }
    return appendExtension(core);
  };

  const formatPhone = (value) => {
    const formatter = typeof window !== 'undefined' && typeof window.formatPhoneDisplay === 'function'
      ? window.formatPhoneDisplay
      : null;
    return (formatter || fallbackFormatPhone)(value);
  };
