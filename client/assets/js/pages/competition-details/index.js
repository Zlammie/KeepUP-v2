...
async function loadSalesSeries(year) {
  ...
  return fetchSalesSeries(competitionId, y);
}

//base price graph loader//
async function loadBasePricesByPlan(anchorMonth) {
  return fetchBasePricesByPlan(competitionId, anchorMonth);
}
...
async function loadPriceScatter(month) {
  return fetchPriceScatter(competitionId, month);
}
...
if (totalLotsInput && totalLotsStat) {
  totalLotsInput.addEventListener('input', () => {
    ...
    totalLotsStat.textContent = Number.isFinite(n) ? n : 'N/A';
...
function asMonthLabel(m) { return m?.month ?? m?.label ?? m?.name ?? 'N/A'; }
...
async function renderQmi() {
  ...
  const fmtCurrency = (n) => formatCurrency(n, { maxFraction: 0 }) || 'N/A';
