    ...
    try {
      ...
      if (linkedIds.length) {
        const priceRecords = await PriceRecord.find({
          ...
          .select('competition month price floorPlan')
          .populate('floorPlan', 'name planNumber')
          .lean();

        const byCompetitionPlan = new Map(); // key -> { compId, planId, plan, monthMap }
        for (const rec of priceRecords) {
          const compId = String(rec.competition);
          const monthKey = typeof rec.month === 'string' ? rec.month.trim() : '';
          if (!ymSet.has(monthKey)) continue;
          const price = toPriceNumber(rec.price);
          if (!Number.isFinite(price)) continue;

          const planObj = rec.floorPlan && typeof rec.floorPlan === 'object' ? rec.floorPlan : null;
          const planId = planObj?._id ? String(planObj._id) : (rec.floorPlan ? String(rec.floorPlan) : null);
          if (!planId) continue;

          const key = `${compId}:${planId}`;
          if (!byCompetitionPlan.has(key)) {
            byCompetitionPlan.set(key, { compId, planId, plan: planObj || null, monthMap: new Map() });
          }
          const entry = byCompetitionPlan.get(key);
          if (!entry.plan && planObj) entry.plan = planObj;
          entry.monthMap.set(monthKey, Math.round(price));
        }

        if (byCompetitionPlan.size) {
          const competitions = await Competition.find({
            _id: { $in: [...new Set([...byCompetitionPlan.values()].map(v => v.compId))] },
            ...baseFilter(req)
          }).select('builderName communityName').lean();
          const compMeta = Object.fromEntries(competitions.map(c => [String(c._id), c]));

          for (const entry of byCompetitionPlan.values()) {
            const info = compMeta[entry.compId] || {};
            const compParts = [clean(info.builderName), clean(info.communityName)].filter(Boolean);
            const compLabel = compParts.length ? compParts.join(' - ') : `Competition ${entry.compId.slice(-4)}`;

            const planInfo = entry.plan || {};
            const planLabel = clean(planInfo.name) || clean(planInfo.planNumber) || `Plan ${entry.planId.slice(-4)}`;

            const series = ymKeys.map(key => {
              const val = entry.monthMap.get(key);
              return Number.isFinite(val) ? val : null;
            });
            if (!series.some(v => v != null)) continue;

            datasets.push({ label: `${compLabel}: ${planLabel}`, data: series });
          }
        }
      }
